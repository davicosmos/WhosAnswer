...
search_button.addEventListener('click', function () {
  if (!clicked) {
    ...
    // ルームIDからルーム名を取得
    axios.get(API_URL + GUEST_NODE.getRoomByCode + '/' + room_id).then((res) => {
...
roomEnterBtn.addEventListener('click', function () {
  if (!clicked) {
    ...
    // ルームIDでルーム名を作成
    axios.post(API_URL + GUEST_NODE.enterRoom, {
      guest_name: tottekitaGuestUserName.value,
      roomIdKensaku: roomIdKensakuTottekuru
    }).then((res) => {
